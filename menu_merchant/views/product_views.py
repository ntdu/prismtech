...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, pk=None, *args, **kwargs):
        try:
            promotion = Product.objects.filter(id=pk).first()

            if not promotion:
                return ApiHelper.custom_response(411, f'No Product with this id: {pk} found')

            promotion.delete()
            return ApiHelper.custom_response(200, 'Delete Product successfully')
        except Exception as e:
            logger.exception(f'Delete Product | get exception {e=}')
            return ApiHelper.custom_response(500, str(e)) 
